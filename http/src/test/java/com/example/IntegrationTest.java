...
class IntegrationTest {
    ...
    private void createGoogleCompany() {
        client.toBlocking().exchange(POST("/", new CompanyRequestBody("Google", "IT", 10, Instant.now(), List.of())));
    }

    private void createAppleCompany() {
        client.toBlocking().exchange(POST("/", new CompanyRequestBody("Apple", "IT", 8, Instant.now(), List.of())));
    }
